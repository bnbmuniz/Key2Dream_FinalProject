...
def get_args():
    ...
    parser.add_argument(       '--out_dir', default='static')
...
def main(text, size, steps, samples):
    a = get_args()
    #(in_img=None, in_txt='pig', in_txt2=None, weight2=0.5, in_txt0=None, out_dir='_out', size=[500, 500], resume=None, opt_step=1, translate=False, multilang=False, save_pt=False, verbose=True, model='ViT-B/32', steps=10, samples=10, lrate=0.05, prog=False, dwt=False, wave='coif2', align='uniform', transform='fast', optimizer='adam', contrast=1.1, colors=1.8, decay=1.5, sharp=0.0, macro=0.4, enforce=0, expand=0, noise=0, notext=0, sync=0, invert=False, sim='mix')
    a.in_txt = text
    a.size = size
    a.steps=steps
    a.samples=samples
    ...
    def train(i):
        ...
        noise = a.noise * torch.rand(1, 1, *params[0].shape[2:4], 1) if a.noise > 0 else None
